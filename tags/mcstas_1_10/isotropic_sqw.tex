...
In order to estimate the probability of interaction of a neutron through the sample, using the linear attenuation $\mu = \rho \sigma$, we need the scattering cross sections for the scattering range.
...
\begin{equation}
\hat{\sigma} = \sigma \iint \frac{S(q,\omega) q}{2 k_i^2} dq d\omega \simeq \frac{\sigma}{2 k_i^2} \int_0^{2k_i} q S(q) dq
\end{equation}
...
\begin{eqnarray}
\hbar \omega &=& E_i - E_f = \frac{\hbar^2}{2m}(k_i^2 - k_f^2) \label{eq:w-transfer_here} \\
\vec q &=& \vec k_i - \vec k_f \label{eq:q-transfer_here}
\end{eqnarray}
...
From the energy conservation law Eq. (\ref{eq:w-transfer_here}), we select randomly one of the two roots, $k_f^+$ and $k_f^-$.
The scattered wave vector is noted : $\vec{k}_f = k_f \vec{\hat k}_s$ where $\vec{\hat k}_s$ is a unit vector.\\
Since we only know the norm of the scattering vector $\vec{q}$, the momentum conservation law Eq. (\ref{eq:q-transfer_here}) may be expressed as
