...
\item Component instances in an instrument source file may be \texttt{GROUP}ed into 
    ...
    neutron, the rest will be skipped. This is useful for multi monochromators
...
\item In both components and instruments, the \texttt{FINALLY} section, that was
    ...
    \texttt{FINALLY} (with the same function as before), and a new \texttt{SAVE}
    section. This latter is executed at simulation end (just before the
    \texttt{FINALLY}), but also each time an intermediate save is required (e.g. 
    when a 'kill -USR2 \$pid' is used under Unix). 
...
\item It is possible to save 3D data arrays, by calling the DETECTOR\_OUT\_3D macro.
    (handled as 2D by \verb+mcplot+). \index{Library!Run-time!DETECTOR\_OUT}
