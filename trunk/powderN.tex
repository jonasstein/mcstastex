...
\component{Powder\_N}{System}{$r$, $h$, $\sigma_{\rm abs}$,
  $\sigma_{\rm inc}$, $V_0$, $f_{\rm pack}$, filename, format, DW}{}{validation in progress}
...
Data files of type \verb'lau' and \verb'laz' in the \MCS\ distribution data directory are self-documented in their header. They do not need any additional parameters to be used, as in the example:
\begin{verbatim}
...
\subsection{Geometry and physical properties}
...
corresponding to one Debye-Scherrer cone reads \cite[ch.3.6]{squires}, with $V$ being the total sample volume:
\begin{equation}
\sigma_{\rm cone}
