...
\component{Single\_crystal}{Kristian Nielsen}{$x_{width}, y_{height}, z_{thick}$,$\vec a, \vec b, \vec c, \Delta d/d$, mosaic, reflections}{$\sigma_{abs}, \sigma_{inc}$, ...}{partially validated, centered}
...
may also be used (see Table \ref{t:powders-data}, page \pageref{t:powders-data}), but as the equivalent $\vec q$ vectors are superposed, not
