...
When running a Monte Carlo, the meaningfull quantities are obtained by integrating random events into a single value (e.g. flux), or onto an histogram grid. The theory \cite{James80} shows that the accuracy of these estimates is a function of the space dimension $d$ and the number of events $N$. For large numbers, the central limit theory shows that the relative error varies as $1/\sqrt{N}$. However, the exact expression depends on the random distributions.
...
\begin{table}
  ...
  {\let\my=\\
    ...
    \hline
    \end{tabular}
    \caption{Accuracy estimate as a function of the number of statistical events, obtained with \MCS.}
    \label{t:mc_accuracy}
  }
  \end{center}
\end{table}
