...
\item when using an homogeneous computer cluster, each simulation (including scan
  steps) may be computed in parallel using MPI (using \texttt{mcrun
    --mpi=Nb\_Nodes}). We highly recommend this method which shows
  excellent performance (but requires the user to install MPI).
\item when using SMP machines (multi processor machines) \MCS\ may
  ...
  mechanism. Even though most systems now support multi-threading (i.e. do not require specific installation), the performance gain is very limited compared to MPI.
\end{itemize}
...
\end{itemize}
The MPI support requires that MPICH (recommended), or alternatively LAM-MPI, is installed on a set of nodes.
This usually also requires properly setup \texttt{ssh} connections and keys as indicated in the ssh grid system (Section \ref{s:ssh-grid}).
...
\item \MCS\ with MPI support was demonstrated to run efficiently up to 10 machines. Above this value, communication issues between nodes may arise, depending on the architecture and network capabilities, particularly when running scans of parameters.\index{Bugs}
\end{itemize}
...
The method is based on a parallel execution of threads. This introduces quite large overhead and dramatically reduces efficiency. Thus, we do NOT recommend using it. Further developments are under way.
