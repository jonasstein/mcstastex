...
\begin{table} \index{Data formats|textbf}
  \begin{center}
    ...
    \begin{tabular}{|p{0.24\textwidth}|c|p{0.7\textwidth}|}
      ...
      \texttt{NeXus} & .nx5 & NeXus data files (HDF). All simulation results are stored in a unique compressed binary file. This format requires to have NeXus installed, and to use NEXUS after the INITIALIZE keyword in the instrument decription.\\
...
The NeXus format~\cite{nexus_webpage} is a platform independent HDF binary data file. To have \MCS\ use it, the HDF and NeXus libraries must have been installed. To link the instrument with these libraries, the keyword \verb+NEXUS+ \emph{must} follow the \verb+INITIALIZE+ section in the instrument description (see Section \ref{s:nexus}). All results are saved in a single file, containing 'groups' of data. To view such files, install and use HDFView. This Java viewer can show content of all detectors, including metadata (attributes). Basic detector images may also be generated.
...
\item when using SMP machines (multi processor machines) \MCS\ may
  ...
  mechanism. Even though most systems now support multi-threading (i.e. do not require specific installation), the performances are limited compared to MPI.
\end{itemize}
