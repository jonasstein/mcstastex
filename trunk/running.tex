...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{|p{0.24\textwidth}|c|p{0.7\textwidth}|}
      ...
      \texttt{Tripoli} &  & Tripoli 4 neutron events file format. Use \verb+Tripoli_input+/\verb+Tripoli_output+ components. \\
      \texttt{Vitess} &  & Vitess neutron events file format. Use \verb+Vitess_input+/\verb+Vitess_output+ components. \\
      \hline
    \end{tabular}
...
\index{MPI|textbf}\index{Grid computing|textbf}
...
\begin{itemize}
\item when scanning instrument parameters (using \texttt{mcrun -N}),
...
\subsection{Distribute mcrun scans (grid)} 
  ...
  \item{ \texttt{ssh} access from the master node (where McStas is
      ...
      password. These keys should be generated using the command \texttt{ssh-keygen}.}
  \item{The machine names listed in the file \texttt{.mcstas-hosts} in
      your home directory on the master node, one node per line.}
  \end{itemize}
  Each of the scan steps is executed on distant machines, sending the executable with 
  \emph{scp}, executing single simulations, and then retrieving individual scan steps on
  the master machine.
  ...
  this is a unix only option in \texttt{mcgui} and \texttt{mcrun}, it {\bf does not} work
  on windows.
...
To enable parallel computation, compile McStas ouput C-source file
...
\item \verb'--mpi-machines=<file>': defines a text file where the
  nodes which are to be used for parallel computation are listed; by
  default, \verb'mcrun' will look at \verb'$HOME/.mcstas-hosts'
\end{itemize}
...
\begin{verbatim}
  mcrun -c --mpi=4 --mpi-machines=machines.list \
...
\item signals are \emph{not} supported while simulating with MPI (since
  asynchrone events cannot be easily transmitted to all nodes)
...
\item[Distribute mcrun scans (grid)] activates the distribution of the scan
  ...
  about how to define and access the grid.
