...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{|p{0.35\textwidth}|p{0.6\textwidth}|}
      ...
      \hline
      \texttt{--threads}
        & This option will distribute the simulation over threads on multi-core/cpu machines (requires OpenMP multi threading to be available). Performances are good.\\
      \hline
...
\item when scanning instrument parameters (using \texttt{mcrun --multi -N}),
  it is possible to distribute each scan step on a list of computers (see section \ref{s:ssh-grid}).
...
\item when using SMP machines (multi core/processor machines) \MCS\ may
  ...
  mechanism, using the \verb+OpenMP+ library. We recommend this method on multicore/cpu machines (see section \ref{s:openmp}).
\end{itemize}
...
\end{itemize}
The MPI support requires that MPICH (recommended), or alternatively LAM-MPI or OpenMPI, is installed on a set of nodes.
This usually also requires properly setup \texttt{ssh} connections and keys as indicated in the ssh grid system (Section \ref{s:ssh-grid}). Some OpenMPI implementations do not recognize multi-cores as separate nodes. In this case, you should use MPICH, or the OpenMP threading mechanism (see section \ref{s:mpi}).
...
Larger tests have shown that the speedup is proportional to the number of nodes, and there is no significant overhead up to at least 100 nodes. The efficiency of \MCS\ w.r.t. parallel computing is thus close to ideal.

It is possible to use the MPI and OpenMP threading together with command \verb+mcrun --mpi --threads ...+ on multi-core clusters (see below).
c
\subsection{McStas/MPI Bugs and limitations}\index{Bugs}

\begin{itemize}
\item Signals are \emph{not} supported while simulating with MPI (since
  asynchroneous events cannot be easily transmitted to all nodes).
...
\item When reading external data files during the TRACE (specially virtual sources) all nodes might access the same data, potentially producing wrong results. We do not recommend using MPI when reading virtual sources.
\item \MCS\ with MPI support was demonstrated to run efficiently up to 100 machines. Above this value, communication issues between nodes may arise, depending on the architecture and network capabilities, particularly when running scans of parameters.\index{Bugs}
\item OpenMPI sometimes does not count multi-cores as separate nodes. We then recommend to use the OpenMP threading mechanism on these systems (see below).
\end{itemize}

\subsection{Parallel computing (multi-threading)}
\label{s:openmp}
Multi-threading using OpenMP is very efficient on multi-core machines. This parallelization does not require any special configuration except a recent C compiler version (gcc 4.2, icc/icpc, pgcc, xlc) to be specified at \MCS\ install/configuration with option e.g. \verb+--with-cc=gcc-4.2+. The number of nodes is set automatically to the number of local cores/cpu. 

To benefit from this mode, use \verb+mcrun --threads ...+. When available, the option will show up in the \verb+mcgui+ Run dialog, and eventually force re-compilation of the instrument if needed. 

It is possible to use the MPI and OpenMP together with command \verb+mcrun --mpi --threads ...+ to benefit entirely from multi-core/cpu clusters. 
