...
\item[\bf B]{\verb+icc+ or the Intel c compiler is available for Linux, Mac
    ...
    {\bf a factor of 2 faster} than the identical simulation run using \verb+gcc+. To use \verb+icc+ with \MCS\, set the environment variables 
    \begin{itemize}
      \item{\verb+MCSTAS_CC=icc+}
      \item{\verb+MCSTAS_CFLAGS="-g -O2 -wd177,266,1011,181"+}
    \end{itemize}
    To use \verb+icc+ with MPI (see Section \ref{s:run-mpi})
...
\item The ENHANCE keyword may artificially repeat events reaching specified positions in the
