...
meta-language and are edited either by a using your favourite editor
...
A typical usage example would be, for instance, to save data during a simulation, plot or analyze it, and decide to interupt the simulation earlier if the statistics has been achieved.
...
\item Divide simulation into parts using a file for saving or generating neutron
      ...
      part. Use the Virtual\_input and Virtual\_output for this technique.
\item Use source optimizers like the components Source\_adapt or
      ...
      computation.
\item Complex components usually take into account small effects in a simulation,
...
For instance, for the \verb+prisma2+ we could write a function for Matlab (see section~\ref{s:analyze} for details about the Matlab data format) in order to sutdy the effects of the $TT$ parameter:
\begin{verbatim}
  function y = instr_value(p)
    ...
    eval(s);        % we could also use 'statistics' field
...
The list of available \MCS\ front-end programs may be obtained from the \verb+mcdoc -t+ command.
...
This front-end currently only works with the PGPLOT plotter, but ports for Matlan and Scilab may be written in the future.
...
The last example will list the name and action of all \MCS\ tools.
...
ans =
          ...
          ...
\end{verbatim}      
The latter exemple accesses the data from the 'monitor1.y\_kz' file written by the 'monitor1' component in the 'test' instrument during the './mcstas' simulation.
