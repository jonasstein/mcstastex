...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{|p{0.24\textwidth}|c|p{0.7\textwidth}|}
      ...
      {\it Tripoli} &  & \hbox{Tripoli 4 neutron events file
      format.} \hbox{Use
        \verb+Virtual_tripoli4_input+/\verb+Virtual_tripoli4_output+}
      components. \\
...
\subsection{Translating and merging \MCS\ results files (all formats)}
\label{s:mcformat}
\index{Tools!mccformat|textbf}
\index{Tools!Matlab} \index{Tools!Scilab} \index{Tools!Octave} \index{Tools!IDL} \index{Tools!HTML}
...
\begin{quote}
  \texttt{mcformat {\it file \my dir} -d {\it target\_dir} --format=TARGET\_FORMAT}
\end{quote}
...
An additional \verb+--merge+ options may be used to merge similar files, e.g. obtained from grid systems.

The analysis of data files works by searching keywords inside data files (e.g. 'Source' for the source instrument description file). If some filenames or component names match these keywords (e.g. using a file 'Source.psd'), the extracted information may be wrong.\index{Bugs}
