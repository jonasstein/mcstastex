...
\begin{description}
\item[Plot current/other results] Plot current simulation results and other results.
\item[Online plotting of results] installs a DSA key to be used for ssh clustering and MPI (see Section \ref{s:run-mpi}).
\item[Dataset convert/merge] Opens a GUI to the \verb+mcformat+ tool, in order to convert datasets to other formats, merge scattered dataset (e.g. from successive or grid simulations), and assemble scan sets.
\item[Shortcut keys] displays the shortcut keys used for running and editing instruments.
\item[Install DSA key] installs a DSA key to be used for ssh clustering and MPI (see Section \ref{s:run-mpi}).\index{MPI}\index{Grid computing}
\end{description}
