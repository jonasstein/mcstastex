...
\item when using a set of nodes (grid, cluster or multi-cores), it is possible to distribute simulations on a 
  ...
  merged after completion. This method is very efficient, and only requires SSH to be installed/configured.
...
\label{s:ssh-grid}
  ...
  \item{Execute \texttt{mcrun} or \texttt{mcgui} as normally, appending \texttt{--multi=NB} or
      \texttt{--grid=NB} will split each simulation onto $NB$ nodes across the machines in
  ...
  results on the master machine. These are then merged. As nodes execute completely
  separated simulations, it is not possible to send messages or interrupt on-going
  computations. However, when peforming simulation scans, it is possible to stop the scan
  as soon as the on-going computation step ends.
  ...
  machines (grids), as well as clusters. For Windows users,
  we recommend the installation of the PuTTY SSH client.
...
This usually also requires properly setup \texttt{ssh} connections and keys as indicated in the ssh grid system (Section \ref{s:ssh-grid}). Some OpenMPI implementations do not recognize multi-cores as separate nodes. In this case, you should use MPICH, or the SSH grid.
...
The MPI support is especially suited on clusters. As an alternative, the SSH grid presented above (section \ref{s:ssh-grid}) is very flexible and requires no configuration.
...
\item When reading external data files during the TRACE (specially virtual sources) 
  ...
  NOT recommend using MPI when reading virtual sources.
\item \MCS\ with MPI support was demonstrated to run efficiently up to 100 machines. 
\end{itemize}
