...
using the 'prec.' entry box. Finally, run the simulation. The optimum
...
\item[prec.] sets the required, final precision when in optimization
  mode. 
...
section~\ref{s:mcrun}, similarly for the integrated intensity on one-
and two-dimensional monitors).

Both one- and two-dimensional monitor output by default start with a
