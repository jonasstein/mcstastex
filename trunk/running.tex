...
{\bf Windows users note:} It is a known problem that some of
the \MCS tools do not support filenames or directories with spaces. %FIXME What about dots?
...
ActiveState {\bf Perl 5.10}. 
%(Note that as of \MCS 1.10, all needed
...
\caption{Left: Output from \texttt{mcdisplay} with PGPLOT backend.
  ...
  program. Right: The PGPLOT time-of-flight option. See section \ref{s:mcdisplay} for details.}
...
\item SGI machines running Irix with the options
  \verb|-Ofast -o32 -w|
\item identify the CPU during compilation and optimize the final program for it\\ \verb+--march=native -O2+
\end{itemize}
...
\end{lstlisting} \index{Tools!Scilab}
and for IDL
\begin{lstlisting}
    idl> s=mcstas()
    idl> s=mcstas(/plot)
...
\begin{figure}[htb!]
  \begin{center}
    \includegraphics[width=0.4\textwidth]{figures/mcgui-config.png}
  \end{center}
\caption{The File/Preferences dialog in \texttt{mcgui}.} 
...
\begin{lstlisting}
mcdisplay -pScilab ./Samples_vanadium.out ROT=90+
\end{lstlisting}
or (\verb+csh+/\verb+tcsh+ syntax)
\begin{lstlisting}
setenv MCSTAS_FORMAT Scilab
mcdisplay ./Samples_vanadium.out ROT=90
\end{lstlisting}
...
The frontend \verb+mcplot+ (mcplot.pl on Windows) is a program that produces
...
\end{lstlisting} \index{Tools!Matlab} \index{Tools!Scilab}
...
\begin{lstlisting}
component Vitess\_input
component Compo
component Vitess\_output
\end{lstlisting}
...
\begin{lstlisting}
mcformat file | dir -d target dir --format=TARGET_FORMAT
\end{lstlisting}
% FIXME that looks strange
...
Both HTML and XML formats are available. The former may be viewed using any web browser showing data sets as VRML objects. XML data sets may be browsed for instance using Internet Explorer (Windows and Mac OS) or Firefox, GXMLViewer and KXMLEditor (under Linux).
...
\texttt{mcgui} supports all of these methods.
...
\item the merge is at worst linear against the number of computers:
  ...
  \item logarithmic time cost: ${\cal O}(\log{p})$ when not saving an event list
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item Some header of output files might contain minor errors. % What does that mean? FIXME
...
\item Interacting with a running simulation (USR1 and USR2 signals) is disabled with MPI.
\end{itemize}
