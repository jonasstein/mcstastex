...
ments as well as polarised neutrons, and includes a library of standard
components with in total around 90 components. These enable to simulate all kinds of neutron scattering instruments (diffractometers, triple-axis, reflectometers, time-of-flight, small-angle, back-scattering, spin-echo,...).
...
({\em e.g.} the Spallation Neutron Source, SNS~\cite{sns_webpage} and the European
...
The \MCS\ project had four main objectives that determined the design of
the \MCS\ software.
...
user may write an intricate simulation of a complex analyser such as
the one of the RITA spectrometers, and expect other users to easily
...
The \MCS\ system consists of the following major
parts:
\begin{itemize}
\item A short list of new features introduced in this \MCS\ release appears in chapter ~\ref{c:changes}
\item A brief introduction to using \MCS\ is given in section~\ref{s:brief}.
\item How to obtain the \MCS\ software is described in section~\ref{s:obtain}.
\item Section~\ref{s:install} explains how to compile
  and install the \MCS\ compiler and associated files, while
  section~\ref{s:running} explains how to run the compiler to produce
...
\item The \MCS\ meta-language is described in chapter~\ref{s:kernel}. This
  chapter also describes a library of kernel functions and definitions
  that aid in the writing of simulations.
\item The \MCS\ component library contains a collection of
  ...
  and on the \MCS\ web-page~\cite{mcstas_webpage}. A short overview of these
...
\item A collection of example instrument definitions is described in
  chapter~\ref{s:instrument}.%, with source code given in appendix~\ref{instcode}.
\item A number of front-end programs are used to run the
...
found in appendix~\ref{s:terminology}, and a list of library calls that may be used in
