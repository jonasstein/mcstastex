...
The \verb+read_table-lib+ provides functionalities for reading text (and binary) data files. To use this library, add a \verb+%include "read_table-lib"+ in your component definition DECLARE or SHARE section. Tables are structures of type $t_Table$.
...
\begin{verbatim}
  t_Table Table;       // declares a t_Table structure
  char file[]="BeO.trm";  // a file name
  double x,y;

  Table_Read(&Table, file, 1);  // initializes and reads the first numerical block
  Table_Info(Table);            // display table informations
  ...
  x = Table_Index(Table, 2,5);  // reads the 3rd row, 6th column element
                                 // of the table. Indexes start at zero in C.
  y = Table_Value(Table, 1.45,1);  // looks for value 1.45 in 1st column (x axis)
...
This other example for text files shows how to read many data blocks in a text file:
\begin{verbatim}
  t_Table *Table;       // declares a t_Table structure array
  long     n;
  double y;

  Table = Table_Read_Array("file.dat", &n); // initializes and reads the all numerical block
  n = Table_Info_Array(Table);     // display informations for all blocks (also returns n)

  y = Table_Index(Table[0], 2,5);  // reads in 1st block the 3rd row, 6th column element
