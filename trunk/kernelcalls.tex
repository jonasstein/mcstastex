...
\item *$Table$ = {\bf Table\_Read\_Array}($filename$, &$blocks$) extracts all numerical blocks of $filename$ into a returned Table array. The number of read blocks is returned in the $blocks$ integer.
\item {\bf Table\_Free\_Array}(&$Table$) Free the Table array.
\item {\bf Table\_Info\_Array}(&$Table$) Gives information about the Table array.
\item {\bf Table_Value2d}($Table$, $X$, $Y$) Uses 2D linear interpolation on a Table, from (X,Y) coordinates and returns the corresponding value.
\end{itemize}
