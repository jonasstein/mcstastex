...
\index{Library!Shared|see{Library/Component/share}}
...
\item {\bf INDEX\_CURRENT\_COMP} is the number (index) of the
       current component  (1 is first component of instrument).
...
\item {\bf SCATTERED} is a variable set to 0 when entering
  ...
  This may be used in the \verb+EXTEND+ sections (always executed) to branch
...
\item {\bf rot\_set\_rotation}$(Rotation t, \phi_x, \phi_y, \phi_z)$ Get transformation for rotation first $\phi_x$ around x axis, then $\phi_y$ around y, then $\phi_z$ around z. $t$ should be a 'Rotation' ([3][3] 'double' matrix).
\item {\bf rot\_mul}$(Rotation t1, Rotation t2, Rotation t3)$ makes $t3 = t1 . t2$.
\item {\bf rot\_copy}$(Rotation dest, Rotation src)$ makes $dest = src$ for Rotation arrays.
\item {\bf rot\_transpose}$(Rotation src, Rotation dst)$ makes $dest = src^t$.
\item {\bf rot\_apply}$(Rotation t, Coords a)$ returns a Coord structure which is $t.a$
\end{itemize}
...
\item {\bf randvec\_target\_circle}(\&$v_x$, \&$v_y$, \&$v_z$, \&$d\Omega$,
  ...
  in $d\Omega$. This routine was previously called randvec\_target\_sphere
  (which still works).
...
\item {\bf Table\_Read\_Offset\_Binary}(\&$Table$, $filename$, $type$, $block$, \&$offset$, $n_{rows}$, $n_{columns}$) does the same as Table\_Read\_Offset, but also specifies the $type$ of the file (may be "float" or "double"), the number $n_{rows}$ of rows to read, each of them having $n_{columns}$ elements. No header should be present.
...
\item {\bf Table\_Value}($Table, x, n$) looks for the closest $x$ value in the first column (index 0), and extracts in this row the $n$-th element. The first column is thus the 'x' axis for the data.
\end{itemize}
...
\begin{verbatim}
    ...
    char file="great.dat";% a file name
    double x,y;
...
\section{Adaptative sampling Library}
\index{Library!adapt\_tree-lib}

This library is currently only used by the components Source\_adapt and Adapt\_check. It performs adaptative sampling of neutrons for simulation efficiency optimization.
Refer to this component for implementation examples, and ask the authors for more details.
