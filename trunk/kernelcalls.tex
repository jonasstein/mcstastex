...
\item {\bf ROT\_A\_COMP}$(comp)$ and
  ...
  0, 1, or 2. 
\item {\bf INDEX\_CURRENT\_COMP} is the first number (index) of the
...
\item {\bf rot\_mul}$(Rotation t1, Rotation t2, Rotation t3)$ performs $t3 = t1 . t2$.
\item {\bf rot\_copy}$(Rotation dest, Rotation src)$ performs $dest = src$ for Rotation arrays.
\item {\bf rot\_transpose}$(Rotation src, Rotation dst)$ performs $dest = src^t$.
...
\item {\bf Table\_Read\_Offset\_Binary}(\&$Table$, $filename$, $type$,
  ...
  of them having $n_{columns}$ elements. No header should be present
  in the file.
